...
EXPORT_ROOT = REPO_ROOT / "notebooks" / "week_21" / "labelstudio_export_pattern_positive_followup_1000"
PROJECT_TITLE_PREFIX = "f1000_"
USER_EMAIL = "benjamin.benji20+label-studio@gmail.com"
EXPORT_BATCH_ID = "week21_pattern_positive_followup_1000"
...
def update_tracking_project_id(path: Path, dataset_key: str, project_id: int, project_title: str) -> None:
    if not path.exists():
        return
    ...
    if not rows:
        return
    for column in ("label_studio_project_id", "label_studio_project_title"):
        if column not in fieldnames:
            fieldnames.append(column)

    changed = False
    for row in rows:
        if row.get("dataset_key") == dataset_key and row.get("export_batch") == EXPORT_BATCH_ID:
            row["label_studio_project_id"] = str(project_id)
            row["label_studio_project_title"] = project_title
            changed = True
    if changed:
...
def main() -> None:
    ...
    for summary in summary_rows:
        ...
        if len(tasks) != exported_count:
            raise RuntimeError(f"{dataset_key}: tasks file has {len(tasks)} rows, summary has {exported_count}")

        base_title = PROJECT_TITLE_PREFIX + dataset_key
        title = unique_project_title(Project, base_title)
        description = (
            "Week 21 pattern-positive follow-up ERP export. "
            f"Batch={EXPORT_BATCH_ID}; dataset={dataset_key}; exported_count={exported_count}; "
            f"positive_sort_variables={summary.get('positive_sort_variables', '')}; source={dataset_dir}"
        )
        ...
        update_tracking_project_id(EXPORT_ROOT / "already_classified_tracking.csv", dataset_key, project.id, title)
        update_tracking_project_id(dataset_dir / "classified_combinations.csv", dataset_key, project.id, title)
        ...
        imported_rows.append(
            {
                ...
                "project_title": title,
                "task_count": len(task_instances),
                "positive_sort_variables": summary.get("positive_sort_variables", ""),
