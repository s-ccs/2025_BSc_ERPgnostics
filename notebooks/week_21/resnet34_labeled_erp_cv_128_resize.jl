...
const OUTPUT_DIR = joinpath(NOTEBOOK_DIR, "outputs", "resnet34_labeled_erp_cv_128_resize")
const MODEL_NAME = "resnet34_pretrained_labeled_erp_binary_resize128"
const MODEL_DEPTH = 34
const K_FOLDS = parse(Int, get(ENV, "WEEK21_RESNET34_FOLDS", "5"))
const TRAIN_EPOCHS = parse(Int, get(ENV, "WEEK21_RESNET34_EPOCHS", string(Generalization.TRAIN_EPOCHS)))
const TRAIN_LR = parse(Float32, get(ENV, "WEEK21_RESNET34_LR", string(Generalization.TRAIN_LR)))
const GLOBAL_SEED = parse(Int, get(ENV, "WEEK21_RESNET34_SEED", "20260501"))
const TARGET_TRIALS = parse(Int, get(ENV, "WEEK21_TARGET_TRIALS", "150"))
const TARGET_SIZE = (
    parse(Int, get(ENV, "WEEK21_RESNET34_TARGET_HEIGHT", "128")),
    parse(Int, get(ENV, "WEEK21_RESNET34_TARGET_WIDTH", "128")),
)
const NO_CLASS_CHUNKS_PER_ORIGIN = parse(Int, get(ENV, "WEEK21_NO_CLASS_CHUNKS_PER_ORIGIN", "1"))
const TRAIN_BATCHSIZE_GPU = parse(Int, get(ENV, "WEEK21_RESNET34_BATCHSIZE_GPU", string(Generalization.TRAIN_BATCHSIZE_GPU)))
const TRAIN_BATCHSIZE_CPU = parse(Int, get(ENV, "WEEK21_RESNET34_BATCHSIZE_CPU", string(Generalization.TRAIN_BATCHSIZE_CPU)))
const PREDICT_BATCHSIZE = parse(Int, get(ENV, "WEEK21_RESNET34_PREDICT_BATCHSIZE", string(Generalization.PREDICT_BATCHSIZE)))
...
function run_resnet34_cv(sample_df::DataFrame;
    ...
    for fold in 1:K_FOLDS
        ...
        model, pretrained_params_loaded = Generalization.build_resnet_single_channel_pretrained(MODEL_DEPTH)
        model = device(model)
...
function run_experiment(;
    ...
    k_folds == K_FOLDS || error("K_FOLDS is constant in this script; set WEEK21_RESNET34_FOLDS before running.")
    ...
    println("Training and validating ResNet34 with ", k_folds, "-fold CV.")
    cv = run_resnet34_cv(sample_df; nepochs = nepochs, lr = lr, seed = seed)
    ...
    write_json(joinpath(output_dir, "run_config.json"), Dict(
        ...
        "model_name" => MODEL_NAME,
        "architecture" => "ResNet34 single-channel, ImageNet-pretrained first convolution projected to 1 channel",
        "metalhead_resnet_api" => "https://fluxml.ai/Metalhead.jl/stable/api/resnet/",
        "metalhead_resnet_constructor" => "Metalhead.ResNet(34; pretrain=false, inchannels=1, nclasses=1000), weights loaded from resnet34-IMAGENET1K_V1 and first convolution projected to one channel",
        ...
        "target_size" => TARGET_SIZE,
        "preprocessing" => "sort -> zscore_timepoints -> Gaussian smoothing -> resize to 128x128",
        "preprocessing_pipeline" => "gaussian_reference",
        ...
        "train_batchsize_gpu" => TRAIN_BATCHSIZE_GPU,
        "train_batchsize_cpu" => TRAIN_BATCHSIZE_CPU,
        "predict_batchsize" => PREDICT_BATCHSIZE,
