...
const WEEK21_DIR = joinpath(REPO_ROOT, "notebooks", "week_21")
const NOTEBOOK_DIR = joinpath(REPO_ROOT, "notebooks", "week_23")
const ANNOTATIONS_CSV = joinpath(WEEK21_DIR, "labelstudio_annotations_all.csv")
const OUTPUT_DIR = joinpath(NOTEBOOK_DIR, "outputs", "augmentation_inverse_sort_polarity")
const MODEL_NAME = "resnet18_pretrained_labeled_erp_inverse_sort_polarity_binary"
const K_FOLDS = parse(Int, get(ENV, "WEEK23_RESNET18_FOLDS", "5"))
const TRAIN_EPOCHS = parse(Int, get(ENV, "WEEK23_RESNET18_EPOCHS", string(Generalization.TRAIN_EPOCHS)))
const TRAIN_LR = parse(Float32, get(ENV, "WEEK23_RESNET18_LR", string(Generalization.TRAIN_LR)))
const GLOBAL_SEED = parse(Int, get(ENV, "WEEK23_RESNET18_SEED", "20260512"))
const TARGET_TRIALS = parse(Int, get(ENV, "WEEK23_TARGET_TRIALS", "200"))
const TARGET_SIZE = Generalization.TARGET_SIZE
const NO_CLASS_CHUNKS_PER_ORIGIN = parse(Int, get(ENV, "WEEK23_NO_CLASS_CHUNKS_PER_ORIGIN", "1"))
...
function stable_seed(parts...)
    seed_u64 = xor(time_ns(), UInt64(hash((parts...,))))
    seed = Int(seed_u64 & UInt64(typemax(Int)))
...
function zscore_timepoints_local(data_time_trials::AbstractMatrix)
    ...
    μ = mean(x; dims = 2)
    σ = std(x; dims = 2, corrected = true)
    σ_safe = ifelse.(Float32.(σ) .== 0f0, 1f0, Float32.(σ))
    return Float32.((x .- Float32.(μ)) ./ σ_safe)
end

function sorted_order_for_variant(events_trials::DataFrame, sort_col::Symbol; inverse_sort::Bool)
    @assert sort_col in propertynames(events_trials) "Sort column $(sort_col) missing."
...
function pre_resize_augmented_image(data_time_trials::AbstractMatrix, events_trials::DataFrame, sort_col::Symbol;
    ...
    order = sorted_order_for_variant(events_trials, sort_col; inverse_sort = inverse_sort)
    data_ordered = Float32.(data_time_trials[:, order])
    inverse_polarity && (data_ordered .*= -1f0)

    data_z = zscore_timepoints_local(data_ordered)
    # ERP image convention: rows = trials (y-axis), columns = time (x-axis).
    return Float32.(permutedims(data_z, (2, 1)))
end

function preprocess_fixed_trial_image(data_time_trials::AbstractMatrix, events_trials::DataFrame, sort_col::Symbol, variant)
...
function assign_stratified_folds!(sample_df::DataFrame; k::Int = K_FOLDS, seed::Int = GLOBAL_SEED)
    ...
    for cls in sort(unique(cellstr.(sample_df.erp_class)))
        ...
        base_ids = unique(Int.(sample_df.base_sample_id[cls_indices]))
        base_ids = shuffle(rng, base_ids)

        for base_id in base_ids
            idxs = findall(i -> cellstr(sample_df.erp_class[i]) == cls && Int(sample_df.base_sample_id[i]) == base_id, 1:nrow(sample_df))
            sort!(idxs; by = i -> Int(sample_df.augmentation_variant_index[i]))
            ...
            used_folds = Set{Int}()
            variant_order = shuffle(rng, idxs)
            for idx in variant_order
                aug = cellstr(sample_df.augmentation_name[idx])
    ...
    for base_id in unique(Int.(sample_df.base_sample_id))
        idxs = findall(==(base_id), Int.(sample_df.base_sample_id))
        assigned = Int.(sample_df.fold[idxs])
        @assert length(unique(assigned)) == length(assigned) "Augmentation variants for base_sample_id=$(base_id) were assigned to duplicate folds."
    end
    return sample_df
end
...
function run_experiment(;
    ...
    k_folds == K_FOLDS || error("K_FOLDS is constant in this script; set WEEK23_RESNET18_FOLDS before running.")
    k_folds >= length(AUGMENTATION_VARIANTS) || error("k_folds must be >= $(length(AUGMENTATION_VARIANTS)) so variants from the same base image can use distinct folds.")
    ...
    augmented_summary_df = combine(groupby(sample_df, [:erp_class, :binary_label, :augmentation_name]), nrow => :count)
    sort!(augmented_summary_df, [:binary_label, :erp_class, :augmentation_name])
    ...
    write_json(joinpath(output_dir, "run_config.json"), Dict(
        ...
        "k_folds" => k_folds,
        "fold_policy" => "stratified by the 7 manual labels and augmentation type; the four variants from each base mod-split image are assigned to distinct folds when k_folds >= 4",
        "augmentation_policy" => "sort trials, build floor(n / target_trials) round-robin mod-split chunks, distribute remainder trials one per round into a separate remainder chunk, fill that remainder chunk to target_trials with deterministic random unique trials from the same origin; positive rows keep all chunks, no_class rows keep one deterministic chunk by default; each kept chunk is rendered as reference, inverse_sort, inverse_polarity, and inverse_sort_inverse_polarity",
        "target_trials" => target_trials,
        "target_trials_source" => "WEEK23_TARGET_TRIALS environment variable, default 200",
        ...
        "n_base_mod_images" => length(unique(Int.(sample_df.base_sample_id))),
