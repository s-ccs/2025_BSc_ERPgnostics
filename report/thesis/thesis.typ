...
Regarding the second question, training on manually labelled real ERP images works well. A pretrained ResNet18 reaches 0.918 balanced accuracy on the labelled pool, performance improved most with Gaussian smoothing and by pattern-preserving augmentation such as trial slicing with inverse sort and polarity. At 64x64 the model keeps most of this accuracy at a fraction of the training cost, so the patterns are detectable both reliably and with modest computational requirements.
