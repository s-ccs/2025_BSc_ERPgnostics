...
#let abstract = [
  Electroencephalography (EEG) records neural activity at millisecond temporal resolution. This produces data with high granularity. To reduce noise, event-related responses commonly get aggregated by averaging repeated trials @Jung2001 @Light2010. What these approaches cannot show is how the response varies across trials. An event-related potential (ERP) image preserves the single-trial structure by stacking sorted trials into a two-dimensional image matrix. This can reveal recurring visual patterns which may indicate cognitive effects @Jung2001. Manual inspection of these images does not scale to studies with many subjects, EEG input channels, and sorting variables. Labelled data for training automatic detectors are scarce. This thesis investigates whether convolutional neural networks (CNNs) can detect such visual patterns and whether simulated images can reduce the need for manually labelled real data.

  This thesis builds a simulation pipeline that generates sigmoid-shaped pattern and pattern-free ERP images. These simulated images are used to train a CNN model and validated against manually labelled data. While this result is more significant than random guessing, the accuracy decreases on other real datasets. Further, the CNN reaches perfect accuracy on held-out simulated data. This difference shows a measurable gap between simulated and real data. Adjusting the simulator parameters to better match the real images narrows this gap without closing it. Varying these parameters widely and at random transfers more reliably than the more guided search strategies tested in this thesis.
...
This visualisation is useful to extract more information than the common aggregating approaches. In EEG experiments with eye-tracking, neighbouring fixations and saccades can often occur close together in time. Therefore, the response to one event may overlap with the response to the next event. This overlap can make ERP image patterns appear shifted, widened, or just harder to interpret. Thus, a pattern in an ERP image can therefore be a clue to a cognitive effect, to event overlap, or to a preprocessing issue. As a result, ERP images are useful as a first diagnostic view, often followed by more explicit overlap modelling or regression-based analysis @Dimigen2011Coregistration, @Ehinger2019Unfold.
...
If the ERP image screening task is addressed with a machine-learning approach, labelled training data become the main bottleneck. ERP images sourced from recordings are heterogeneous due to differences in experimental setups, and are mostly not labelled with regard to our interest in finding visual patterns. Manual annotation remains the preferred option for real ERP images, but is expensive. Therefore, such evaluation is too limited to train a deep image classifier reliably from scratch for use in a real-world analysis workflow @Roy2019. Simulation complements these labels by creating labelled examples under known assumptions. However, real annotated data remains necessary for validation and transfer testing.

This thesis focuses on the automated approach to find visual patterns in ERP images. The goal is to detect interpretable ERP image morphologies of interest, instead of explaining the underlying neural process or origin. Deep learning is a plausible tool for this image-based task, but only if the label bottleneck and the gap between simulated and real ERP images are handled explicitly.

Hence, we also use simulation to create labelled training data under controlled assumptions. UnfoldSim.jl is used to simulate continuous event-based EEG-like time series, which can then be rendered as ERP images @Schepers2025. This setup motivates the research questions and hypotheses set out in the next section.
